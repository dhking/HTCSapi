...
(function ($) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload: function (opt, serverCallBack) {
            ...
            //uploadBtnArray.push(webUploader);
            debugger;       
            ...
            webUploader.on('fileQueued', function (file) {
                ...
                createBox($fileInput, file, webUploader, 1);
    ...
    $.fn.extend({
		...
		*	serverCallBack回调函数 每个文件上传至服务端后,服务端返回参数,无论成功失败都会调用 参数为服务器返回信息;
		*/
        diyUpload1: function (opt, serverCallBack) {
            if (typeof opt != "object") {
                alert('参数错误!');
                return;
            }
            ...
            webUploader.on('fileQueued', function (file) {
                ...
                createBox($fileInput, file, webUploader, 2);
    ...
    //创建文件操作div;	
    function createBox($fileInput, file, webUploader, lei) {
        file.name = CurentTime(file.id) + "." + gettype(file.name);
        var file_id = file.id;
        ...
        var li1 = '<li id="fileBox_' + file_id + '" class="diyUploadHover" > \
					<div class="viewThumb"></div> \
					<div class="diyCancel1"></div> \
					<div class="diySuccess"></div> \
					<div class="diyFileName">'+ file.name + '</div>\
					<div class="diyBar"> \
							<div class="diyProgress"></div> \
        ...
        if (lei == 2) {
            $parentFileBox.children('.fileBoxUl').append(li1);
        }
        ...
        var $diyCancel = $fileBox.children('.diyCancel').one('click', function () {
            ...
            var name = null;
            $.each(fileArr, function (i, v) {
                name = addimage0.indexOf(v.name);
            })
            addimage0.baoremove(name);
            removeLi($(this).parent('li'), file_id, webUploader);

        });
        var $diyCancel1 = $fileBox.children('.diyCancel1').one('click', function () {
            var fileArr = webUploader.getFiles('complete');
            debugger;
            var name = null;
            $.each(fileArr, function (i, v) {
                name = addimage1.indexOf(v.name);
            });
            addimage1.baoremove(name)
            removeLi($(this).parent('li'), file_id, webUploader);
